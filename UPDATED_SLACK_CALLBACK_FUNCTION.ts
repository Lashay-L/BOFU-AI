...
serve(async (req: Request) => {
  ...
  let returnUrl = `${Deno.env.get('FRONTEND_URL') || 'http://localhost:5173'}/admin`;
  ...
  try {
    ...
    if (state) {
      try {
        ...
        if (stateData?.returnPath) {
          returnUrl = stateData.returnPath; // Use the full URL from the state
        }
      } catch (_e) {
    ...
    if (error) {
      ...
      let redirectUrl = `${returnUrl}${urlSeparator}slack_error=${encodeURIComponent(error)}`
      
      // Include company context in error redirect if available
      if (stateData?.companyId && stateData?.companyName) {
        const stateForRedirect = btoa(JSON.stringify({
          companyId: stateData.companyId,
          companyName: stateData.companyName
        }));
        redirectUrl += `&state=${encodeURIComponent(stateForRedirect)}`;
      }
    ...
    if (!code || !stateData || (stateData.type !== 'admin_connection' && state !== 'admin_connection')) {
      ...
      const urlSeparator = returnUrl.includes('?') ? '&' : '?';
      const redirectUrl = `${returnUrl}${urlSeparator}slack_error=invalid_request`
      return Response.redirect(redirectUrl, 302)
    }
    ...
    if (!supabaseUrl || !serviceRoleKey) {
      ...
      const urlSeparator = returnUrl.includes('?') ? '&' : '?';
      const redirectUrl = `${returnUrl}${urlSeparator}slack_error=configuration_error`
      return Response.redirect(redirectUrl, 302)
    }
    ...
    if (!tokenData.ok) {
      ...
      const urlSeparator = returnUrl.includes('?') ? '&' : '?';
      const redirectUrl = `${returnUrl}${urlSeparator}slack_error=${encodeURIComponent(tokenData.error || 'token_exchange_failed')}`
      return Response.redirect(redirectUrl, 302)
    }
    ...
    if (updateError) {
      ...
      const urlSeparator = returnUrl.includes('?') ? '&' : '?';
      const redirectUrl = `${returnUrl}${urlSeparator}slack_error=database_error`
      return Response.redirect(redirectUrl, 302)
    }
    ...
    const urlSeparator = returnUrl.includes('?') ? '&' : '?';
    let redirectUrl = `${returnUrl}${urlSeparator}slack_success=true&team=${encodeURIComponent(tokenData.team.name)}`
    
    // Include company context in redirect if available
    if (stateData?.companyId && stateData?.companyName) {
      const stateForRedirect = btoa(JSON.stringify({
        companyId: stateData.companyId,
        companyName: stateData.companyName
    ...
    const urlSeparator = returnUrl.includes('?') ? '&' : '?';
    const redirectUrl = `${returnUrl}${urlSeparator}slack_error=server_error`
    return Response.redirect(redirectUrl, 302)
  }
})